...
import com.doctracker.basic.ui.model.WorksheetTableModel;
 ...
 */
public class ExcelDataImporter extends ExcelDataHandler<Doc> {
...
//            };
    ...
    public ExcelDataImporter(App app) {
        super(app, "Excel Data Importer", 700, 700);
        this.app = app;
        ...
        this.textHandler = app.getTextHandler();
    }
    
    @Override
    public void execute(File file, String sheetName) {
        
        super.execute(file, sheetName);
        
        new ExcelDataVerifier(app).execute(file, sheetName);
    }
    
    @Override
    protected Doc handleRow(Doc previousDoc, Sheet sheet, Cell [] cells, int row, Set<Integer> failedRows) {
...
// DOPRI, Comms, LOG & COMMS, AFIT Comdt
    ...
    private List<Appointment> promptConfirmApptOrCreateNew(App app, Sheet sheet, String apptStr, int row, int col) throws TaskExecutionException {
    ...
    private SelectAppointmentOrCreateNewPanel createAndShowSelectAppointmentUI(Sheet sheet, int row, int col) {
        ...
        app.getUI().positionFullScreen(frame);
        
        frame.pack();
        frame.setVisible(true);
        
        final ColumnWidth columnWidths = app.getUI().getColumnWidths(Task.class, null);
        app.getUI().getTableCellSizeManager().updateCellSizes(
                container.getTable(), tableModel, columnWidths, null);
